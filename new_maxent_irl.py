...
def compute_state_visition_freq(P_a, gamma, trajs, policy, deterministic=True):
  ...
  inputs:
    ...
    trajs   list of litrajsst of Steps - collected from expert
  ...
  for s in range(N_STATES):
    for t in range(T-1):
      if deterministic:
        mu[s, t+1] = sum([mu[pre_s, t]*P_a[pre_s, int(policy[pre_s]), s] for pre_s in range(N_STATES)])
      else:
...
def maxent_irl(feat_map, P_a, gamma, trajs, lr, n_iters):
  ...
  # theta = np.zeros((feat_map.shape[1],))
  ...
  for episode in trajs:
    for cur_state, _, _ in episode:
      feat_exp += feat_map[cur_state,:]
  ...
  for iteration in range(n_iters):
    ...
    theta += lr * grad
    #lr -= lr_const
    np.save("theta", arr=theta)

  np.save("recent_theta", arr=theta)
  rewards = np.dot(feat_map, theta)
